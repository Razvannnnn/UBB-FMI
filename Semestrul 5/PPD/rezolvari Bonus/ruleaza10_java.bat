...
set JAVA_PATH=D:\UBB INFO\UBB INFO - ANUL III (2025-2026)\Semestrul 5\PPD\Teme-Lab\Lab1\rezolvari Bonus\rezolvare_bonus_java\src

REM === Fisierul CSV unde salvam timpii ===
set OUTPUT_CSV=D:\UBB INFO\UBB INFO - ANUL III (2025-2026)\Semestrul 5\PPD\Teme-Lab\Lab1\rezolvari Bonus\rezolvare_bonus_java\outputs_lab\timpi_paralel.csv
...
echo nr_iteratie,timp_blocuri,timp_linear,timp_cyclic > "%OUTPUT_CSV%"
...
for /l %%i in (1,1,10) do (
    ...
    for /f "tokens=*" %%t in ('java -cp . Main %%i') do (
        ...
        if !count! equ 2 set "timp_linear=%%t"
	if !count! equ 3 set "timp_cyclic=%%t"
    )

    echo %%i,!timp_blocuri!,!timp_linear!,!timp_cyclic! >> "%OUTPUT_CSV%"
)
