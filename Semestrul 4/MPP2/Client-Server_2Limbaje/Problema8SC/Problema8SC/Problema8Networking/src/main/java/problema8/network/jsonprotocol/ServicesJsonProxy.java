...
public class ServicesJsonProxy implements IService {
    ...
    @Override
    public List<ChildDetails> getDetailsForAllChildren() throws ProbExceptions {
        ...
        if (response.getType() == 0) {
            String jsonData = gsonFormatter.toJson(response.getData());
            Type listType = new TypeToken<List<ChildDetails>>() {}.getType();
            List<ChildDetails> detailsList = gsonFormatter.fromJson(jsonData, listType);
            return detailsList;
        }
        return null;
    }
